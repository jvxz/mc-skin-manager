const interactiveStyles = {
  ...
  variant: {
    default:
      'bg-primary/85 border focus-visible:border-primary/50 hover:bg-primary/90 border-primary active:bg-primary/85 text-primary-foreground glow-primary',
    destructive:
...
const popoverStyles = {
  ...
  item: [
    ...
    "focus:bg-muted focus:text-accent-foreground text-foreground data-[disabled]:text-muted-foreground [&_svg:has([disabled])]:text-muted-foreground [&_svg:not([class*='size-'])]:size-4 gap-1.5 data-[variant=destructive]:text-destructive-foreground data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive-background data-[variant=destructive]:*:[svg]:!text-destructive-foreground dark:data-[variant=destructive]:focus:bg-destructive/40 relative flex cursor-default items-center p-1 px-2 text-sm outline-hidden transition-all select-none focus-visible:ring-0 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[inset]:pl-8 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  ],
}
